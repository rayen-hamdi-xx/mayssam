...
export default function TimePicker(props){
    ...
    return (
        <div onClick={()=>props.setOpen(!props.open)} tabIndex={"0"} className={classNames("flex flex-col cursor-pointer justify-between w-full  desktop2:min-w-[7rem]  h-12 tablet:h-16 laptop:h-16 desktop2:h-full desktop:h-full p-[0.5rem] desktop2:p-[0.75rem]  shadow-input bg-neutral-100 hover:shadow-input_hover_red_50 hover:bg-red-50 relative   rounded-[5px] ",{" p-[0.5rem]":!props.open})}>
                   <AnimatePresence>
                    {
                     !props.open ?
                     <>
                    ...
                    <div className="flex  items-center gap-[0.375rem] desktop:gap-[0.5rem] desktop2:gap-[0.5rem] ">
                        <I_CLOCK className="h-[12px] w-[12px] tablet:h-[16px] tablet:w-[16px] laptop:h-[16px] laptop:w-[16px] dekstop:h-[18px] desktop:h-[18px] desktop2:h-[20px] desktop2:w-[20px] "/>
